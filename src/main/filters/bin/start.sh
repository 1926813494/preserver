...
MAIN_CLASS="com.yaoxun.preserver.Main"

DEPLOY_DIR=`pwd`
CONF_DIR=$DEPLOY_DIR/conf

SERVER_NAME=`sed '/application.name/!d;s/.*=//' conf/config.properties | tr -d '\r'`
SERVER_PORT=`sed '/application.port/!d;s/.*=//' conf/config.properties | tr -d '\r'`
LOGS_FILE=`sed '/application.log.file/!d;s/.*=//' conf/config.properties | tr -d '\r'`
